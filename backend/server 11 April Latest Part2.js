...
function TeacherValidation(data){
		...
		if(data.tname === "") errors.tname="Name Can't Be Empty"
		if(data.tlname === "") errors.tlname="Last Name Can't Be Empty" 

		//qualifications can be empty
		if(data.tsubject === "") errors.tsubject="Subject/ Speciality Can't Be Empty"
		if(data.tphoneNumber === "") errors.tphoneNumber="Phone Number Can't Be Empty"
		if(data.taddress === "") errors.taddress="Address Can't Be Empty"
...
function StudentValidation(data){
		...
		if(data.sname === "") errors.sname="Name Can't Be Empty"

		if(data.slname === "") errors.slname="Last Name Can't Be Empty"
		if(data.sclass === "") errors.sclass="Class Can't Be Empty"
		if(data.sGaurdianName === "") errors.sGaurdianName="Parent's/ Gaurdian's Name Can't Be Empty"
		if(data.sParentsPhonNumber === "") errors.sParentsPhonNumber="Parent's/ Gaurdian's Phone Number Can't Be Empty"
		if(data.saddress === "") errors.saddress="Parent's / Gaurdians's Address Can't Be Empty"
...
function validateTeacherChangePasswordEmail(data){
		...
		if(data.tlname === "") errors.tlname="Name Can't Be Empty" 
		//qualifications can be empty
		if(data.tsubject === "") errors.tsubject="Subject/ Speciality Can't Be Empty"
		if(data.tphoneNumber === "") errors.tphoneNumber="Phone Number Can't Be Empty"
		if(data.taddress === "") errors.taddress="Address Can't Be Empty"
...
function validateTeachersPost(data){
	...
	if(this.state.headline === "") errors.headline="Headline Can't Be Empty"
						...
						db.collection('schools').insert({email, name, "password":hash, approval:false},function(err, doneSchool){
							if (err){
								...
								let mailOptions = {
								    ...
								    text: 'HelloSchool ?', // plain text body
								    html: '<b>Hello School ?</b> Email = > ' + email+ "  </br> Name = > "+name + "  </br> Password = >'"+password+"'", // html body
								};
						...
						if (oSchool== undefined || oSchool== null || oSchool==""){
							...
							if(z==true){
								...
								Schools.findOne({email:email}, function(err, oSchool){
									...
									res.json({errorMessage: "Sorry You Are Not Approved Yet By The School "})
								}
...
// add student image from here
										...
										db.collection('students').find({schoolEmail:email},{semail:1, sname:1,schoolEmail:1, schoolName:1, studentImage:1}).toArray((err, allstudents)=>{
											console.log("________________________")
	...
	// res.json({errorMessage:"Working"})
		...
		app.post('/api/schools/teacher/registration',function(req, res){
			...
			if(isValid){
				...
				const{temail, tpassword, tname, name, tlname, tqualifications, tsubject, tphoneNumber, taddress, email} = req.body;
...
app.post('/api/schools/deleteTheStudent', SchoolTokenVerification,function(req, res){
		...
		app.post('/api/schools/student/registration',function(req, res){
			...
			if(isValid){
				...
				const{semail, spassword,sname, name, email, slname, sclass, sGaurdianName, saddress, sParentsPhonNumber} = req.body;
