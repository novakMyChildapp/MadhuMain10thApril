			...
			const isValid = Object.keys(errors).length===0
				...
				if(isValid){
						...
						this.props.saveTeacherPostOnStudent({temail, tpassword, tname, headline, description ,semail}).then(
							// ()=>{console.log("HEREEEE")}
							(response)=>{this.setState({done:true}) ;console.log(response) ;console.log("%$^$^$%^%$^$%^$%^$%^%$^$%$%^%$") ;if(response.teacherPostsOnStudentsTimeLine.errorMessage && response.teacherPostsOnStudentsTimeLine.errorMessage!==undefined && response.teacherPostsOnStudentsTimeLine.errorMessage!==null && response.teacherPostsOnStudentsTimeLine.errorMessage!==''){this.setState({errorState:true, errorMessage: response.teacherPostsOnStudentsTimeLine.errorMessage, loading:false})}else{ this.setState({loading:false, done:true, shouldRedirect:true,headline:"", description:""})}},
				...
				<div>	
					<form id="contact" onSubmit={this.handleSubmit}>
					  <div className="containerTimeline">
					    ...
					    <div className={classnames('form-group', {"has-error":!!this.state.errors.headline})}><input className="timelineInput" type="text" name="headline" value={this.state.headline} onChange={this.handleChange} id="headline" placeholder="Headline" /><span className="errorM">{this.state.errors.headline}</span></div>
					    <div className={classnames('form-group', {"has-error":!!this.state.errors.description})}><textarea className="timelineInput" type="text" name="description" value={this.state.description} onChange={this.handleChange} id="description" placeholder="Description"></textarea><span className="errorM">{this.state.errors.description}</span></div>
	...
	<div className="well">
      <div className="media">
	          ...
	          <ul className="list-inline list-unstyled"> 
	            ...
	            <li>|</li>
	            <span><i className="glyphicon glyphicon-comment"></i>{this.props.data.comments && this.props.data.comments!=null && this.props.data.comments!=undefined && this.props.data.comments<0? this.props.data.comments: " 0" } Comments</span> 
	            <li>|</li>
	          </ul>
	       </div>
       </div>
