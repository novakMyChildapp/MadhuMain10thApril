...
class SchoolForm extends React.Component{
	...
	if(isValid){
		...
		this.props.saveSchoolLogin({email, name, password,cpassword }).then(
			(response)=>{this.setState({done:true}) ; if(response.school.errorMessage && response.school.errorMessage!==undefined && response.school.errorMessage!==null && response.school.errorMessage!==''){this.setState({errorState:true, errorMessage: response.school.errorMessage, loading:false})}else{ this.setState({loading:false, done:true, shouldRedirect:true})}},
