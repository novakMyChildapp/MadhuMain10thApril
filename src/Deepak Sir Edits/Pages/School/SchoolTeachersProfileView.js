...
import {FetchTeacherProfile, TeacherProfileUpdation} from '../../actions/SchoolActions'
...
class SchoolTeachersProfileView extends React.Component{
	...
	handleSubmit = (e)=>{
		...
		if(this.state.tlname === "" || this.state.tlname==undefined) errors.tlname="Last Name Cant Be Empty" 
			console.log(this.state.tlname)
			//qualifications can be empty
		if(this.state.tsubject === "" || this.state.tsubject==undefined) errors.tsubject="Subject/ Speciality Cant Be Empty"
		if(this.state.tphoneNumber === "" || this.state.tphoneNumber==undefined) errors.tphoneNumber="Phone Number Cant Be Empty"
		if(this.state.taddress === "" || this.state.taddress==undefined) errors.taddress="Address Cant Be Empty"

		if(this.state.tpassword ==="" || this.state.tpassword==undefined) errors.tpassword="Password Cant Be Empty"
		if(this.state.tcpassword ==="" || this.state.tcpassword==undefined) errors.tcpassword="Password Cant Be Empty"
		...
		if(isValid){
			...
			const {temail, tpassword, tcpassword, email, password, tname, tlname, name, tqualifications, tsubject, tphoneNumber, taddress} = this.state
			this.setState({loading: true})
			this.props.TeacherProfileUpdation({temail,tpassword, tname, tlname, tqualifications, tsubject, tphoneNumber, taddress, email, password, name }).then(
				(response)=>{debugger;this.setState({done:true});console.log("%%%%%%%%%%%%%");console.log(response) ; if(response.errorMessage && response.errorMessage!==undefined && response.errorMessage!==null && response.errorMessage!==''){this.setState({errorState:true, errorMessage: response.errorMessage, loading:false})}
	...
	componentDidMount(){
		...
		this.setState({loading: true})
		this.props.FetchTeacherProfile({email, password, id }).then(
...
export default connect(null ,{FetchTeacherProfile, TeacherProfileUpdation})  (SchoolTeachersProfileView)