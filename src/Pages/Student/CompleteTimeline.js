...
// debugger
		...
		if(comment === "" || comment.trim() === "") e.target.lastElementChild.children[1].children[2].innerHTML="Comment Can't Be Empty"
		if(comment === "" || comment.trim() === "") errors.comment="Comment Can't Be Empty"
			// e.target.lastElementChild.children[1].children[2].innerHTML=
...
var idAndTemail =e.target.className.split("-*-")
		...
		if(isValid){
			...
			this.props.StudentCommenting({semail, spassword, temail, postId, comment, studentToken }).then(
				...
				(err) => err.response.json().then(({errors})=> {debugger; console.log("here"); this.setState({errors, loading:false}); e.target.lastElementChild.children[1].children[2].innerHTML="Comment Can't Be Empty"})
			)
				...
				{this.props.data.paths && this.props.data.paths!=null && this.props.data.paths.map((post, i) => <ImagesRow key={i} data2={post} s={this.state} />)}
								...
								{ this.props.s.shouldRedirect ? reloadme:"" }
								  ...
								  <div className="containerTimeline">
									...
									<span className="errorM">{/*this.props.s.errors.comment*/}</span></div>					   }
...
class ImagesRow extends React.Component{
	render(){
		return(
				<div>
					<h4 className="uploaded-images-heading">Uploaded Image</h4>
